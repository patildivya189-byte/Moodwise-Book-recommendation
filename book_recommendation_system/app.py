...
st.caption("Collaborative filtering demo — User-Based, Item-Based, SVD, and Hybrid")
...
with st.sidebar:
    st.header("Settings")
    user_id = st.selectbox("Select a user", options=list(matrix.index), index=0)
    method_name = st.radio("Recommendation method", list(METHODS.keys()))
...
rated_books = books[books.book_id.isin(already_rated.index)].copy()
rated_books["rating"] = rated_books.book_id.map(already_rated)
st.dataframe(rated_books[["title", "author", "genre", "rating"]], hide_index=True)

st.subheader(f"Recommended for user {user_id} — {method_name}")
fn = METHODS[method_name]
results = fn(matrix, user_id, books, top_n=top_n)

if results:
    result_df = pd.DataFrame(results)[["title", "author", "genre", "predicted_rating"]]
    st.dataframe(result_df, hide_index=True)
else:
    st.info("No recommendations available for this user with the current settings.")
st.divider()
